...
# We should make assumptions to simplify the data. We have already made some assumptions where we have chosen specific data to focus on, and have removed certain data with a N/A. 

# If we did not make assumptions, the data would be overwhelming and it would be difficult to make causal interpretations.

# Ask why is someone interested in asking the certain questions about the data, which could be understanding why certain classes have a drop in enrollment, and addressing these issues.  


After we have a model, Temperance suggests we should:

# We should recognize the limitation of our model. These enrollment results are dependent upon the time that we are collecting the data.

# When we are making assumptions based on average results, we should be aware that these conclusions may not account for individual variations per class.  
...
```{r}
newenrol <- inner_join(sep_new, sep_old, by = c("id", "name", "department", "title"), suffix = c("_new", "_old")) %>%
